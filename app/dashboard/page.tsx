      ...
      <div className="flex">
        ...
        <section className="min-h-screen flex-1 px-6 py-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-5">
            <StatCard title="TOTAL EXTRACTED" value="12,450" chip="+12% vs last week" />
            <StatCard title="QUALIFIED LEADS" value="845" chip="+5% High intent" />
            <StatCard title="ACTIVE CAMPAIGNS" value="12" chip="0% Running smoothly" />
            <StatCard title="RESPONSES" value="342" chip="+4.2% Response rate" />
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            ...
            <Card className="lg:col-span-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-white/45">Lead Quality Distribution</span>
              </div>

              <div className="mt-5 grid gap-4 sm:grid-cols-2 sm:items-center">
                {/* Fake donut (CSS ring) */}
                <div className="mx-auto grid h-40 w-40 place-items-center rounded-full bg-[conic-gradient(#2563ff_0_45%,#22c55e_45%_70%,#64748b_70%_100%)] p-[14px]">
                  <div className="grid h-full w-full place-items-center rounded-full bg-ink-900">
                    <div className="text-center">
                      <div className="text-xl font-semibold">12.4k</div>
                ...
                <div className="space-y-3 text-sm">
                  <LegendDot color="bg-green-500" label="Hot (25%)" sub="Highly Qualified" />
                  <LegendDot color="bg-blue-500" label="Warm (45%)" sub="Engagement detected" />
                  <LegendDot color="bg-slate-500" label="Cold (30%)" sub="No activity" />
                </div>
              </div>
            </Card>
...
function StatCard({ title, value, chip }: { title: string; value: string; chip: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      <div className="text-[11px] font-semibold tracking-wider text-white/45">{title}</div>
...
function ActionTile({ title, desc, highlight }: { title: string; desc: string; highlight?: boolean }) {
  return (
    <button
      type="button"
      className={[
...
function Row({
  ...
  return (
    <tr className="border-b border-white/5 last:border-b-0">
      ...
      <td className="py-4">
        <span className={["inline-flex rounded-full border px-3 py-1 text-xs font-semibold", badge].join(" ")}>
          {status}
        </span>
      </td>
